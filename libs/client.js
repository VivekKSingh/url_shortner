...
var DEFAULT_OPTIONS = {
  url: 'http://api-console.client-testing.adnxs.net',
  sandboxUrl: 'http://api-console.client-testing.adnxs.net'
};
...
Client.prototype.getPublisher = function(publisher, cb) {
  this.authenticate(function(err) {
    if(err) {
      return cb(err);
    }

    return cb(null, new Publisher(publisher, this));
  }.bind(this));
};

Client.prototype.addPublisher = function(publisher, cb) {
  this.authenticate(function(err) {
    if(err) {
      return cb(err);
    }
...
Client.prototype.getDefaultSite = function(publisher, cb) {
  this.authenticate(function(err) {
    if(err) {
      return cb(err);
    }

    return cb(null, new Site(publisher, this));
  }.bind(this));
};

Client.prototype.createPlacement = function(placement, publisher, cb) {
    this.authenticate(function(err) {
        if(err) {
            return cb(err);
        }

        return cb(null, new Placement(placement, publisher, this));
    }.bind(this));
};
